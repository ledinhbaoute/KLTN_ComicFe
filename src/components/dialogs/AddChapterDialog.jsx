      ...
      if (checkfile === 1) {
        ...
        if (results.length === 0) {
          ...
          if (azure_results.length === 0) {
            ...
            toast.error(
              `Nội dung của bạn không được duyệt do chứa các file hình ảnh nhạy cảm sau: ${listFile}, vui lòng thực hiện chỉnh sửa hoặc yêu cầu admin duyệt lại!`,
              { id: toastId }
            );
          }
        } else {
          ...
          <div>
            ...
            {!isAccept && (
              <Button
                variant="contained"
                color="success"
                onClick={handleAddChapterNotAccept}
              >
                Yêu cầu duyệt lại
              </Button>
            )}
